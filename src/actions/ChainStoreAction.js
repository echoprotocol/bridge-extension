...
import { getAssetsBalances } from './BalanceActions';
	...
	ChainStoreCacheNames.forEach(({ origin, custom: field }) => {
		...
		if (value && (field === 'accountsByName')) {
			value.forEach(async (val) => {
				...
				if (val === accountId) {
					const chainAssets = (await fetchChain(accountId)).get('balances');
					const assets = getState().balance.get('assets');

					chainAssets.forEach(async (chainAsset) => {
						const chainBalance = (await fetchChain(chainAsset)).get('balance');

						if (assets.find((asset) => asset.id === chainAsset)) {
							if (assets.find((asset) => asset.balance !== chainBalance)) {
								dispatch(getAssetsBalances(chainAssets.toObject()));
							}
						} else {
							dispatch(getAssetsBalances(chainAssets.toObject()));
						}

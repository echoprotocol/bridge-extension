...
export const getAssetsBalances = (assets) => async (dispatch) => {
	...
	if (assets && Object.keys(assets).length) {
		balances = Object.entries(assets).map(async (asset) => {
			const balance = (await dispatch(EchoJSActions.fetch(asset[1]))).get('balance');
			asset = (await dispatch(EchoJSActions.fetch(asset[0])));
			return { balance, precision: asset.get('precision'), symbol: asset.get('symbol') };
		});
